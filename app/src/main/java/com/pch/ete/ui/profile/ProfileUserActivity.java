...
public class ProfileUserActivity extends BaseActivity implements IApiCallback, PickImage {
    ...
    private Runnable runnableName = new Runnable() {
        ...
        public void run() {
            if (!etFullName.getText().toString().isEmpty())
                ApiCall.getInstance().updateFullName(preference.getAccessToken(), etFullName.getText().toString(), ProfileUserActivity.this);
        }
    };
    ...
    private Runnable runnableRole = new Runnable() {
        ...
        public void run() {
            if (!etRole.getText().toString().isEmpty())
                ApiCall.getInstance().updateUserRole(preference.getAccessToken(), etRole.getText().toString(), ProfileUserActivity.this);
        }
    };
...
//        setPager();
    ...
    void updateUserName() {
        if (etFullName.getText().toString().isEmpty()) {
            ...
            mHandler.postDelayed(runnableName, 700);
        }
    }
