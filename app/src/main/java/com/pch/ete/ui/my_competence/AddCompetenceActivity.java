...
public class AddCompetenceActivity extends BaseActivity implements IApiCallback {
    ...
    CompetenceInfo mCompetenceInfo;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String jsonStr = getIntent().getStringExtra("competence_data");
        if (jsonStr != null) {
            tvTitle.setText(getString(R.string.edit_competence));
            mCompetenceInfo = new Gson().fromJson(jsonStr, CompetenceInfo.class);

            String makeName="", modelName="", skillName = "";
            if(mCompetenceInfo.getMakeData() != null) {
                makeName = mCompetenceInfo.getMakeData().getMakeName();
                tvMakeEquipment.setTag(mCompetenceInfo.getMakeData().getId());
            }
            if(mCompetenceInfo.getModelData() != null) {
                modelName = mCompetenceInfo.getModelData().getModelName();
                tvModel.setTag(mCompetenceInfo.getModelData().getId());
            }
            if(mCompetenceInfo.getSkillData() != null) {
                skillName = mCompetenceInfo.getSkillData().getSkillName();
                tvSkill.setTag(mCompetenceInfo.getSkillData().getId());
            }
            ...
            tvServiceTime.setText(String.valueOf(mCompetenceInfo.getServiceHour()));
        }
    }
    ...
    public void openModelDialog() {
        ...
        builder.setSingleChoiceItems(mModelStr, -1, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                tvModel.setTag(mModelList.get(which).getId());
                dialog.dismiss();
            }
        });
    ...
    public void openSkillDialog() {
        ...
        builder.setSingleChoiceItems(mSkillStr, -1, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                tvSkill.setTag(mSkillList.get(which).getId());
                dialog.dismiss();
            }
        });
    ...
    void onClickAddCompetence() {
        ...
        if (InternetCheck.isConnectedToInternet(this)) {
            ProgressHelper.dismiss();
            ProgressHelper.showDialog(this);
            if (mCompetenceInfo == null) {
                ApiCall.getInstance().addCompetence(
                        preference.getAccessToken(),
                        tvMakeEquipment.getTag().toString(),
                        tvModel.getTag().toString(),
                        tvSkill.getTag().toString(),
                        mCertificatedDocFile,
                        mAuthorizedDocFile,
                        tvServiceTime.getText().toString(),
                        this
                );
            } else {
                ApiCall.getInstance().updateCompetence(
                        preference.getAccessToken(),
                        String.valueOf(mCompetenceInfo.getId()),
                        tvMakeEquipment.getTag().toString(),
                        tvModel.getTag().toString(),
                        tvSkill.getTag().toString(),
                        mCertificatedDocFile,
                        mAuthorizedDocFile,
                        String.valueOf(mCompetenceInfo.getServiceHour()),
                        this
                );
            }
        }
    ...
    public void onSuccess(String type, Response response) {
        ...
        } else if (type.equals("add_competence")) {
            if (response.isSuccessful()) {
